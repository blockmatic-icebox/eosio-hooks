...
var transitReducer = function (state, action) {
    switch (action.type) {
        case 'CONNECT_WALLET_START':
            return __assign(__assign({}, state), { connecting: action.payload.provider, error: false });
        case 'CONNECT_WALLET':
            return __assign(__assign({}, state), { connecting: null, wallet: action.payload.wallet, error: false });
